...
import { ArrowLeft, Clock, MapPin, Pencil, Plus, Trash2, Trophy } from 'lucide-react'
...
type MatchEvent = { id: number; teamId: number; playerId: number | null; playerName: string | null; type: EventType; minute: number | null }
...
type Standing = {
  teamId: number; team: string; played: number; won: number; drawn: number; lost: number
...
type League = {
  id: number; name: string; description: string | null; locationId: number; locationName: string
  seasonStart: string | null; seasonEnd: string | null; status: 'active' | 'finished'
}
    ...
    <Dialog open onClose={onClose} title="Editar liga">
      <form onSubmit={submit} className="flex flex-col gap-4">
        ...
        <Field label="Estado" error={form.errors.status}>
          <Select value={form.data.status} onChange={(e) => form.setData('status', e.target.value as League['status'])}>
        ...
        <div className="mt-1 flex justify-end gap-2">
          ...
          <Button type="submit" variant="lime" disabled={form.processing}>Guardar</Button>
        </div>
      </form>
    </Dialog>
  )
...
function TeamDialog({ leagueId, team, onClose }: { leagueId: number; team: Team | 'new'; onClose: () => void }) {
  const isEdit = team !== 'new'
  const form = useForm({ name: isEdit ? team.name : '', logoUrl: isEdit ? (team.logoUrl ?? '') : '' })
  const submit = (e: React.FormEvent) => {
    e.preventDefault()
  ...
  return (
    <Dialog open onClose={onClose} title={isEdit ? 'Editar equipo' : 'Nuevo equipo'}>
      <form onSubmit={submit} className="flex flex-col gap-4">
        <Field label="Nombre" error={form.errors.name}>
          <Input value={form.data.name} onChange={(e) => form.setData('name', e.target.value)} placeholder="Halcones" required />
        </Field>
        <Field label="URL de logo" error={form.errors.logoUrl} hint="Opcional">
          <Input type="url" value={form.data.logoUrl} onChange={(e) => form.setData('logoUrl', e.target.value)} placeholder="https://…" />
        </Field>
        <div className="mt-1 flex justify-end gap-2">
          <Button type="button" variant="secondary" onClick={onClose}>Cancelar</Button>
          <Button type="submit" variant="lime" disabled={form.processing}>{isEdit ? 'Guardar' : 'Agregar'}</Button>
        </div>
      </form>
    </Dialog>
  )
...
function TeamCard({ team, onEdit }: { team: Team; onEdit: () => void }) {
  ...
  const removePlayer = (id: number) => router.delete(`/dashboard/players/${id}`, { preserveScroll: true })
  const removeTeam = () => {
    ...
    <Card className="flex flex-col p-5">
      <div className="flex items-center justify-between gap-2">
        <div className="min-w-0">
          ...
          <p className="text-xs text-slate-6">{team.players.length} {team.players.length === 1 ? 'jugador' : 'jugadores'}</p>
        </div>
        <div className="flex shrink-0 items-center gap-1">
          <Button variant="ghost" size="icon" onClick={onEdit} aria-label="Editar equipo"><Pencil className="size-3.5" /></Button>
          <Button variant="danger" size="icon" onClick={removeTeam} aria-label="Eliminar equipo"><Trash2 className="size-4" /></Button>
        </div>
      </div>

      <ul className="mt-4 flex flex-col gap-1.5">
        {team.players.length === 0 ? (
          <li className="rounded-lg bg-bone-2 px-3 py-2.5 text-sm text-slate-6">Sin jugadores aún.</li>
        ) : (
          team.players.map((p) => (
            <li key={p.id} className="flex items-center gap-2.5 rounded-lg bg-bone-2 px-3 py-2 text-sm">
              <span className="grid size-6 shrink-0 place-items-center rounded-md bg-chalk text-xs font-semibold tabular-nums text-slate-6">{p.number ?? '–'}</span>
              <span className="flex-1 truncate text-graphite">{p.name}</span>
              <button type="button" onClick={() => removePlayer(p.id)} aria-label="Quitar" className="text-slate-6 transition-colors hover:text-rose-mark">
      ...
      <form onSubmit={addPlayer} className="mt-4 flex items-end gap-2 border-t border-bone-2 pt-4">
        <div className="w-14">
          <Input type="number" min="0" value={form.data.number} onChange={(e) => form.setData('number', e.target.value)} placeholder="#" aria-label="Número" />
        </div>
        <div className="flex-1">
          <Input value={form.data.name} onChange={(e) => form.setData('name', e.target.value)} placeholder="Nombre del jugador" required aria-label="Jugador" />
        </div>
        <Button type="submit" variant="secondary" size="icon" disabled={form.processing} aria-label="Agregar jugador"><Plus className="size-4" /></Button>
      </form>
    </Card>
  )
}
      ...
      <form onSubmit={submit} className="flex flex-col gap-4">
        <div className="grid grid-cols-2 gap-4">
          <Field label="Local" error={form.errors.homeTeamId}>
            <Select value={form.data.homeTeamId} onChange={(e) => form.setData('homeTeamId', e.target.value)} required>
              {teams.map((t) => <option key={t.id} value={t.id}>{t.name}</option>)}
            </Select>
          </Field>
          <Field label="Visitante" error={form.errors.awayTeamId}>
            <Select value={form.data.awayTeamId} onChange={(e) => form.setData('awayTeamId', e.target.value)} required>
              {teams.map((t) => <option key={t.id} value={t.id}>{t.name}</option>)}
            </Select>
          </Field>
        </div>
        ...
        {isEdit && (
          <Field label="Estado" error={form.errors.status}>
            <Select value={form.data.status} onChange={(e) => form.setData('status', e.target.value as MatchStatus)}>
        ...
        <div className="mt-1 flex justify-end gap-2">
          <Button type="button" variant="secondary" onClick={onClose}>Cancelar</Button>
          <Button type="submit" variant="lime" disabled={form.processing}>{isEdit ? 'Guardar' : 'Programar'}</Button>
        </div>
      </form>
    </Dialog>
  )
...
const EVENT_LABEL: Record<EventType, string> = { goal: '⚽ Gol', yellow: '🟨 Amarilla', red: '🟥 Roja' }

function MinutaDialog({ match, teams, onClose }: { match: Match; teams: Team[]; onClose: () => void }) {
  const teamById = useMemo(() => new Map(teams.map((t) => [t.id, t])), [teams])
  const form = useForm({ teamId: String(match.homeTeamId), playerId: '', type: 'goal' as EventType, minute: '' })
  ...
  const removeEvent = (id: number) => router.delete(`/dashboard/match-events/${id}`, { preserveScroll: true })
      ...
      <div className="flex flex-col gap-4">
        <ul className="flex max-h-52 flex-col gap-1.5 overflow-y-auto">
          {match.events.length === 0 ? (
            ...
            match.events.map((ev) => (
              <li key={ev.id} className="flex items-center gap-2 rounded-lg bg-bone-2 px-2.5 py-1.5 text-sm">
                <span className="w-8 shrink-0 text-center tabular-nums text-slate-6">{ev.minute != null ? `${ev.minute}'` : '—'}</span>
                <span className="shrink-0">{EVENT_LABEL[ev.type]}</span>
                <span className="flex-1 truncate text-graphite">
                  {ev.playerName ?? 'Sin asignar'}
                  <span className="text-slate-6"> · {ev.teamId === match.homeTeamId ? match.homeTeam : match.awayTeam}</span>
                </span>
                <button type="button" onClick={() => removeEvent(ev.id)} aria-label="Quitar" className="text-slate-6 hover:text-rose-mark">
                  <Trash2 className="size-3.5" />
                </button>
              </li>
            ))
        ...
        <form onSubmit={add} className="flex flex-col gap-3 border-t border-bone-3 pt-4">
          <div className="grid grid-cols-2 gap-3">
            <Field label="Equipo" error={form.errors.teamId}>
              <Select value={form.data.teamId} onChange={(e) => form.setData({ ...form.data, teamId: e.target.value, playerId: '' })}>
            ...
            <Field label="Tipo" error={form.errors.type}>
              <Select value={form.data.type} onChange={(e) => form.setData('type', e.target.value as EventType)}>
          ...
          <div className="grid grid-cols-[1fr_5rem] gap-3">
            <Field label="Jugador" error={form.errors.playerId}>
              <Select value={form.data.playerId} onChange={(e) => form.setData('playerId', e.target.value)}>
            ...
            <Field label="Min" error={form.errors.minute}>
              <Input type="number" min="0" value={form.data.minute} onChange={(e) => form.setData('minute', e.target.value)} placeholder="45" />
            </Field>
          </div>
          <Button type="submit" variant="lime" disabled={form.processing}><Plus className="size-4" /> Agregar a la minuta</Button>
        </form>
      </div>
    </Dialog>
  )
...
export default function LeagueShow({ league, spaces, teams, matches, standings, scorers, cards }: Props) {
  ...
  const removeMatch = (m: Match) => {
    if (confirm('¿Eliminar este partido?')) router.delete(`/dashboard/matches/${m.id}`, { preserveScroll: true })
  }
  ...
  return (
    <>
      ...
      <Link href="/dashboard/leagues" className="mb-4 inline-flex items-center gap-1 text-sm text-slate-6 hover:text-graphite">
        ...
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div className="min-w-0">
            <div className="flex items-center gap-2">
              ...
              <span className={cn('inline-flex h-6 items-center rounded-md px-2 text-[11px] font-semibold', league.status === 'active' ? 'bg-emerald-mark/15 text-emerald-mark' : 'bg-bone-2 text-slate-6')}>
            ...
            <p className="mt-1 flex flex-wrap items-center gap-x-3 text-sm text-slate-6">
              <span className="inline-flex items-center gap-1"><MapPin className="size-3.5" /> {league.locationName}</span>
              {(league.seasonStart || league.seasonEnd) && (
                <span>{league.seasonStart ? formatDate(league.seasonStart) : '—'} – {league.seasonEnd ? formatDate(league.seasonEnd) : '—'}</span>
              )}
            </p>
          </div>
          <Button variant="secondary" size="sm" onClick={() => setEditLeague(true)}><Pencil className="size-3.5" /> Editar liga</Button>
        </div>
      </Card>
        ...
        {TABS.map((t) => (
          <button
            ...
            className={cn(
              ...
              tab === t.key ? 'bg-chalk text-graphite shadow-sm' : 'text-slate-6 hover:text-graphite'
            )}
          >
            {t.label}
          </button>
        ...
        <div className="space-y-6">
          <Card className="overflow-hidden">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {standings.length === 0 ? (
                    <tr><td colSpan={10} className="px-5 py-10 text-center text-sm text-slate-6">Agrega equipos para ver la tabla.</td></tr>
                  ) : (
                    standings.map((s, i) => (
                      <tr key={s.teamId} className="border-t border-bone-2 transition-colors hover:bg-bone-1/70">
                        <td className="py-3.5 pl-5 pr-2">
                          <span className={cn('inline-flex size-6 items-center justify-center rounded-full text-xs font-semibold tabular-nums', i < 3 ? 'bg-lime-mark/30 text-graphite' : 'text-slate-6')}>{i + 1}</span>
                        </td>
                        <td className="py-3.5 pr-4 font-medium text-graphite">{s.team}</td>
                        <td className="px-3 py-3.5 text-center tabular-nums text-slate-6">{s.played}</td>
                        <td className="px-3 py-3.5 text-center tabular-nums text-slate-6">{s.won}</td>
                        <td className="px-3 py-3.5 text-center tabular-nums text-slate-6">{s.drawn}</td>
                        <td className="px-3 py-3.5 text-center tabular-nums text-slate-6">{s.lost}</td>
                        <td className="px-3 py-3.5 text-center tabular-nums text-slate-6">{s.goalsFor}</td>
                        <td className="px-3 py-3.5 text-center tabular-nums text-slate-6">{s.goalsAgainst}</td>
                        <td className="px-3 py-3.5 text-center tabular-nums text-slate-6">{s.goalDiff > 0 ? `+${s.goalDiff}` : s.goalDiff}</td>
                        <td className="py-3.5 pl-3 pr-5 text-center font-bold tabular-nums text-graphite">{s.points}</td>
                      </tr>
                    ))
                  )}
                </tbody>
          ...
          {hasPlayed && (
            <div className="grid gap-6 lg:grid-cols-2">
              <Card className="p-6">
                <h3 className="mb-4 flex items-center gap-2 text-sm font-semibold text-graphite">⚽ Goleadores</h3>
                {scorers.length === 0 ? (
                  ...
                  <ul className="-my-2 divide-y divide-bone-2 text-sm">
                    {scorers.map((s, i) => (
                      <li key={i} className="flex items-center gap-3 py-2.5">
                        <span className="w-5 shrink-0 text-center text-xs font-semibold tabular-nums text-slate-6">{i + 1}</span>
                        <span className="flex-1 truncate text-graphite">{s.player} <span className="text-slate-6">· {s.team}</span></span>
              ...
              <Card className="p-6">
                ...
                {cards.length === 0 ? (
                  ...
                  <ul className="-my-2 divide-y divide-bone-2 text-sm">
                    {cards.map((c, i) => (
                      <li key={i} className="flex items-center gap-3 py-2.5">
                        <span className="flex-1 truncate text-graphite">{c.player} <span className="text-slate-6">· {c.team}</span></span>
      ...
      {tab === 'calendario' && (
        <div className="space-y-4">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <p className="text-sm text-slate-6">{matches.length} {matches.length === 1 ? 'partido' : 'partidos'}</p>
            <Button variant="lime" size="sm" disabled={!enoughTeams} onClick={() => setMatchDialog('new')}>
              <Plus className="size-4" /> Programar partido
            </Button>
          </div>
          {!enoughTeams && <p className="text-sm text-slate-6">Necesitas al menos 2 equipos para programar partidos.</p>}
              ...
              {matches.map((m) => (
                <Card key={m.id} className="p-5">
                  <div className="flex flex-wrap items-center gap-x-4 gap-y-1.5 text-xs text-slate-6">
                    ...
                    <span className="inline-flex items-center gap-1"><Clock className="size-3.5" /> {timeRange(m.startTime, m.endTime)}</span>
                    <span className="inline-flex items-center gap-1"><MapPin className="size-3.5" /> {m.spaceName}</span>
                    <span className={cn('inline-flex h-5 items-center rounded-md px-2 text-[11px] font-semibold', MATCH_STATUS[m.status].cls)}>{MATCH_STATUS[m.status].label}</span>
                  </div>

                  <div className="mt-4 grid grid-cols-[1fr_auto_1fr] items-center gap-3 sm:gap-5">
                    <span className="truncate text-right font-semibold text-graphite">{m.homeTeam}</span>
                    <span className="rounded-xl bg-bone-2 px-4 py-1.5 text-lg font-bold tabular-nums text-graphite">{m.homeGoals}–{m.awayGoals}</span>
                  ...
                  <div className="mt-4 flex items-center justify-end gap-2 border-t border-bone-2 pt-4">
                    <Button variant="secondary" size="sm" onClick={() => setMinuta(m)}>Minuta</Button>
                    <Button variant="ghost" size="icon" onClick={() => setMatchDialog(m)} aria-label="Editar"><Pencil className="size-3.5" /></Button>
                    <Button variant="danger" size="icon" onClick={() => removeMatch(m)} aria-label="Eliminar"><Trash2 className="size-4" /></Button>
                  </div>
                </Card>
              ))}
            </div>
      ...
      {tab === 'equipos' && (
        <div className="space-y-4">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <p className="text-sm text-slate-6">{teams.length} {teams.length === 1 ? 'equipo' : 'equipos'}</p>
            <Button variant="lime" size="sm" onClick={() => setTeamDialog('new')}><Plus className="size-4" /> Nuevo equipo</Button>
          </div>
          {teams.length === 0 ? (
      ...
      {minuta && <MinutaDialog key={minuta.id} match={minuta} teams={teams} onClose={() => setMinuta(null)} />}
    </>
  )
}
