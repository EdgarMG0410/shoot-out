      ...
      <form onSubmit={submit} className="flex flex-col gap-4">
        ...
        <Field label="Nombre" error={form.errors.name}>
          <Input
            ...
            placeholder="Cancha Norte / Terraza Centro"
            required
          />
        </Field>
        <div className="grid grid-cols-2 gap-4">
          <Field label="Tipo" error={form.errors.type}>
            <Select
              ...
              onChange={(e) => form.setData('type', e.target.value as SpaceType)}
            >
          ...
          {form.data.type === 'cancha' ? (
            <Field label="Tamaño" error={form.errors.size}>
              <Select
                ...
                onChange={(e) => form.setData('size', e.target.value as '5' | '7' | '11')}
              >
        ...
        <Field label="Precio / hora (MXN)" error={form.errors.pricePerHour}>
          <Input
            ...
            min="1"
            step="0.01"
            value={form.data.pricePerHour}
            onChange={(e) => form.setData('pricePerHour', e.target.value)}
            placeholder="450"
            required
          />
        </Field>
        ...
        <div className="grid grid-cols-2 gap-4">
          <Field label="Abre" error={form.errors.openTime} hint="Horario de renta">
            <Input
              type="time"
              value={form.data.openTime}
              onChange={(e) => form.setData('openTime', e.target.value)}
              required
            />
          </Field>
          <Field label="Cierra" error={form.errors.closeTime}>
            <Input
              type="time"
              value={form.data.closeTime}
              onChange={(e) => form.setData('closeTime', e.target.value)}
              required
            />
          </Field>
        </div>
        {isEdit && (
          <Field label="Estado" error={form.errors.status}>
            <Select
              ...
              onChange={(e) => form.setData('status', e.target.value as 'active' | 'blocked')}
            >
...
export function BlockDialog({ space, onClose }: { space: SpaceRow; onClose: () => void }) {
  ...
  return (
    <Dialog
      open
      onClose={onClose}
      title={`Bloquear horario · ${space.name}`}
      description="Marca una franja como no disponible."
    >
      <form onSubmit={submit} className="flex flex-col gap-4">
