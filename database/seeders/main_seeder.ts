...
export default class extends BaseSeeder {
  async run() {
    ...
    await User.updateOrCreate(
      ...
      { fullName: 'Admin Shootout', email: 'admin@shootout.mx', password: 'password123', role: 'admin' }
    )
    ...
    const spaces = [
      { name: 'Cancha Centro', locationId: centro.id, type: 'cancha', size: '5', pricePerHour: 450, capacity: null, photoUrl: FOTO.cancha5, openTime: '08:00', closeTime: '23:00', status: 'active' },
      { name: 'Cancha Techada', locationId: centro.id, type: 'cancha', size: '5', pricePerHour: 500, capacity: null, photoUrl: FOTO.cancha5, openTime: '09:00', closeTime: '22:00', status: 'blocked' },
      { name: 'Terraza Centro', locationId: centro.id, type: 'terraza', size: null, pricePerHour: 800, capacity: 120, photoUrl: FOTO.terraza, openTime: '12:00', closeTime: '23:30', status: 'active' },
      { name: 'Cancha Norte', locationId: norte.id, type: 'cancha', size: '7', pricePerHour: 650, capacity: null, photoUrl: FOTO.cancha7, openTime: '07:00', closeTime: '23:00', status: 'active' },
      { name: 'Estadio Sur', locationId: norte.id, type: 'cancha', size: '11', pricePerHour: 1200, capacity: null, photoUrl: FOTO.cancha11, openTime: '08:00', closeTime: '22:00', status: 'active' },
    ] satisfies {
      name: string
      locationId: number
    ...
    if (Number(leagueCount?.$extras.c ?? 0) === 0) {
      ...
      if (canchaCentro) {
        ...
        await MatchEvent.createMany([
          { matchId: m1.id, teamId: teams['Halcones'].id, playerId: firstPlayer['Halcones'].id, type: 'goal', minute: 12 },
          { matchId: m1.id, teamId: teams['Halcones'].id, playerId: firstPlayer['Halcones'].id, type: 'goal', minute: 34 },
          { matchId: m1.id, teamId: teams['Pumas'].id, playerId: firstPlayer['Pumas'].id, type: 'goal', minute: 50 },
          { matchId: m1.id, teamId: teams['Pumas'].id, playerId: firstPlayer['Pumas'].id, type: 'yellow', minute: 60 },
        ])
